...
class RedisFeatureStore(BaseFeatureStoreOperator):
    """High-performance Redis-backed sliding-window feature store."""
    ...
    def _init_connection(self) -> None:
        ...
        try:
            import redis
            client = redis.Redis.from_url(self.redis_url, decode_responses=True, socket_timeout=0.5)
            client.ping()
            self.client = client
    ...
    async def record_event(
        ...
        """Record an event in the sliding-window sorted set."""
        event_data = {
            "ts": timestamp,
            "amt": amount,
            "ip": ip or "",
        ...
        if self.client:
            ...
            # ZSET member: JSON string of event data, Score: timestamp
            member = json.dumps(event_data)
            self.client.zadd(zset_key, {member: timestamp})
            # Prune events older than 24 hours (86400 seconds) to maintain memory hygiene
    ...
    async def get_sliding_window_features(
        ...
        if self.client:
            zset_key = f"feat:cust:{customer_id}:txs"
            # Query last 1 hour of events for aggregations
